...
import static toy.blockchain.Blockchain.validateBlockHash;

public abstract class ToyBaseServer extends Node implements Server  {
    //    class fpEntry {
//        ForkProof fp;
//        boolean done;
//    }
    ...
    private final ConcurrentHashMap<Integer, ArrayList<Types.subChainVersion>> scVersions;
...
//    private Block recBlock;
    ...
    public ToyBaseServer(String addr, int wrbPort, int id, int channel, int f,
...
//            }
            if (fp.containsKey(currHeight) && fp.get(currHeight)) {
                scVersions.computeIfPresent(currHeight, (k, v) -> {
                    if (v.size() > 0 && !v.get(0).equals(Types.subChainVersion.getDefaultInstance())) {
                        try {
                            adoptSubVersion(currHeight);
                        } catch (IOException e) {
                            logger.error(String.format("[#%d-C[%d]] unable to sync", getID(), channel), e);
...
//        }
    ...
    private void addApprovedTransactions(List<Types.Transaction> txs) {
//        synchronized (approvedTx) {
...
//        }
    ...
    void removeFromProposed(List<Types.Transaction> txs) {
//        proposedTx.removeAll(txs.stream().map(Transaction::getTxID).collect(Collectors.toList()));
    }

    private boolean validateForkProof(Types.ForkProof p)  {
    ...
    private void syncBlockingPhase(int forkPoint) throws IOException {
        while ((!scVersions.containsKey(forkPoint)) ||
            ...
            scVersions.computeIfPresent(v.getForkPoint(), (k, v1)-> {
                if (v1.size() > 0 && v1.get(0).equals(Types.subChainVersion.getDefaultInstance())) {
                    return v1;
                }
                v1.add(v);
...
//        fp.get(forkPoint).add(ForkProof.getDefaultInstance());
    ...
    private void adoptSubVersion(int forkPoint) throws IOException {
        ...
        final Types.subChainVersion[] choosen = new Types.subChainVersion[1];
        final int[] sPoint = {0};
        scVersions.computeIfPresent(forkPoint, (k, v1)-> {
            ...
            choosen[0] = v1.
                    ...
                    filter(v -> v.getV(v.getVCount() - 1).getHeader().getHeight() == max
                            && validateBlockHash(bc.getBlock(forkPoint - (f + 2)), v.getV(0))).
                    findFirst().get();
            ...
            ArrayList<Types.subChainVersion> ret = new ArrayList<>();
            ret.add(Types.subChainVersion.getDefaultInstance());
            return ret;
        });
